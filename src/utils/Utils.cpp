...
cv::Mat Utils::qImageToMat(const QImage &qim)
{
    cv::Mat mat;
...
void Utils::drawImgText(QImage &img, const std::string &text, const std::string &area, const std::string type)
{
    if (text.empty() || area.empty())
    {
        LogWarn("Draw image faile: no text info or area info");
        return;
    }
    QPainter pp(&img);
    QFont font = pp.font();
    ...
    if (type == "c2c3")
    {
        ...
        textVec.push_back(text.substr(0, std::min(text.size(), size_t(12))));
        if (text.size() > 12)
        {
            textVec.push_back(text.substr(12));
        }
        textVec.resize(2);
        int textNum = 0;
        for (const auto &item : areaList)
        {
            ...
            sscanf_s(leftTop.c_str(), "[%d, %d]", &leftTopY, &leftTopX);
            sscanf_s(rightBottom.c_str(), "[%d, %d]", &rightBottomY, &rightBottomX);
            pp.drawRect(leftTopX, leftTopY, rightBottomX - leftTopX, rightBottomY - leftTopY);
            pp.setBrush(QBrush(Qt::green));
            if (textNum != 2)
            {
                pp.drawText(QPointF(leftTopX, leftTopY - 15), textVec[textNum++].c_str());
            }
        }
    }
    else
    {
        ...
        for (const auto &item : areaList)
        {
            ...
            sscanf_s(leftTop.c_str(), "[%d, %d]", &leftTopX, &leftTopY);
            sscanf_s(rightTop.c_str(), "[%d, %d]", &rightTopX, &rightTopY);
            sscanf_s(rightBottom.c_str(), "[%d, %d]", &rightBottomX, &rightBottomY);
            sscanf_s(leftBottom.c_str(), "[%d, %d]", &leftBottomX, &leftBottomY);
            QVector<QPointF> points{QPointF(leftTopX, leftTopY), QPointF(rightTopX, rightTopY),
                                    QPointF(rightBottomX, rightBottomY), QPointF(leftBottomX, leftBottomY)};
...
void Utils::saveVideo(std::vector<cv::Mat> &matMap, std::string &outputPath)
{
    if (matMap.empty())
    {
        LogWarn("save video is empty!");
        return;
    }
    cv::VideoWriter writer_;
    cv::Size frameSize = cv::Size(matMap[0].cols, matMap[0].rows);
    double fps = 30;
    int codec = cv::VideoWriter::fourcc('M', 'J', 'P', 'G'); // 选择编解码器
    writer_.open(outputPath, codec, fps, frameSize);
    for (auto &frame : matMap)
    {
        writer_.write(frame);
    }
    writer_.release();
    LogInfo("finish save video");
}