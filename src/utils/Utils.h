...
class Utils
{
  public:
    ...
    static cv::Mat qImageToMat(const QImage &qim);
    ...
    static void drawImgText(QImage &img, const std::string &text, const std::string &area,
                            const std::string type = ""); // 图像添加文字
    static std::string transferDataFromSql(std::string);
    static uint16_t countSubstrings(const std::string &s, const std::string &&sub);
    static void saveVideo(std::vector<cv::Mat> &, std::string &);
};
