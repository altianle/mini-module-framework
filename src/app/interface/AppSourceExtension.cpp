...
void AppFrame::AppSourceExtension::initFilePath(QString &&appPath)
{
    ...
    pathVideo_ = appPath + "/Video/";
    for (size_t i = 0; i < 5; i++)
    {
        QString tempPath = pathVideo_ + QString("usb_camera_") + QString::number(i);
        if (!qdir.mkpath(tempPath))
        {
            LogWarn("create dir error: {}", tempPath.toStdString());
        }
    }
}
...
QString AppFrame::AppSourceExtension::getFilePath(const ResourcePath path)
{
    ...
    case AppFrame::ResourcePath::PathVideo: {
        retPath = pathVideo_;
        break;
    }
    default:
